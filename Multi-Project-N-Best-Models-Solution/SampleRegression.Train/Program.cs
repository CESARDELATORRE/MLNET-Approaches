...
namespace Sample
{
    class Program
    {
        ...
        private const string MODEL01_RELATIVE_PATH = @"model01.zip";
        private const string MODEL02_RELATIVE_PATH = @"model02.zip";
        private const string MODEL03_RELATIVE_PATH = @"model03.zip";
        ...
        static void Main(string[] args)
        {
            ...
            // 2. Train Models
            (ITransformer model01, string trainerName01) = TrainModel01(mlContext, trainingDataView);
            (ITransformer model02, string trainerName02) = TrainModel02(mlContext, trainingDataView);
            (ITransformer model03, string trainerName03) = TrainModel03(mlContext, trainingDataView);

            // 3. Evaluate quality of Models
            EvaluateModel(mlContext, model01, testDataView, trainerName01);
            EvaluateModel(mlContext, model02, testDataView, trainerName02);
            EvaluateModel(mlContext, model03, testDataView, trainerName03);

            // 4. (Optional) Try a single prediction (Only one case with first row of DataSet)
            TrySinglePrediction(mlContext, model01, testDataView);
            TrySinglePrediction(mlContext, model02, testDataView);
            TrySinglePrediction(mlContext, model03, testDataView);

            // 5. Save model
            SaveModel(mlContext, model01, MODEL01_RELATIVE_PATH);
            SaveModel(mlContext, model02, MODEL02_RELATIVE_PATH);
            SaveModel(mlContext, model03, MODEL03_RELATIVE_PATH);
        ...
        public static (ITransformer model, string trainerName) TrainModel01(MLContext mlContext, IDataView trainingDataView)
        {
        ...
        public static (ITransformer model, string trainerName) TrainModel02(MLContext mlContext, IDataView trainingDataView)
        {
            // Common data process configuration with pipeline data transformations
            IEstimator<ITransformer> dataProcessPipeline = mlContext.Transforms.Categorical.OneHotEncoding(new[] { new OneHotEncodingEstimator.ColumnInfo("vendor_id", "vendor_id"), new OneHotEncodingEstimator.ColumnInfo("payment_type", "payment_type") })
                                      .Append(mlContext.Transforms.Concatenate(DefaultColumnNames.Features, new[] { "vendor_id", "payment_type", "rate_code", "passenger_count", "trip_time_in_secs", "trip_distance" }));
        ...
        public static (ITransformer model, string trainerName) TrainModel03(MLContext mlContext, IDataView trainingDataView)
        {
            // Common data process configuration with pipeline data transformations
            IEstimator<ITransformer> dataProcessPipeline = mlContext.Transforms.Categorical.OneHotEncoding(new[] { new OneHotEncodingEstimator.ColumnInfo("vendor_id", "vendor_id"), new OneHotEncodingEstimator.ColumnInfo("payment_type", "payment_type") })
                                      .Append(mlContext.Transforms.Concatenate(DefaultColumnNames.Features, new[] { "vendor_id", "payment_type", "rate_code", "passenger_count", "trip_time_in_secs", "trip_distance" }));
